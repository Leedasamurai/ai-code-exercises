...
program.version("1.0.0").description("Task Manager CLI");
...
program
  ...
  .action((taskId, dueDate) => {
    if (taskManager.updateTaskDueDate(taskId, dueDate)) {
      ...
      console.log(
        "Failed to update task due date. Task not found or invalid date.",
      );
    }
  });
...
program
  ...
  .option("-p, --priority <priority>", "Filter by task priority")
  .action((filename, options) => {
    try {
      taskManager.exportTasksToCSV(filename, options);
